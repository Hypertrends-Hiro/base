  ...
  return (
    <div className="space-y-5 text-sm leading-relaxed text-foreground">
      ...
      <section>
        ...
        <p className="mt-1"><strong>Risks:</strong> Technical failures may interrupt care. In emergencies, always call 911 or visit an emergency room. Telemedicine is not a substitute for in-person emergency care.</p>
      </section>
      <section>
...
export function SectionInformedConsent({ answers, onAnswer, onNext, onBack, direction }: Props) {
    ...
    >
      <button
        ...
        className={cn(
          ...
          consentTelemedicine ? "border-primary bg-primary/10" : "border-border bg-background/50 hover:border-primary/50 hover:bg-background"
        )}
      >
        <Checkbox checked={consentTelemedicine} onCheckedChange={() => onAnswer("consentTelemedicine", !consentTelemedicine)} className="pointer-events-none shrink-0" aria-hidden />
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-[11px] font-bold uppercase tracking-widest text-foreground">Telemedicine Informed Consent</span>
          <a href="#" onClick={(e) => { e.preventDefault(); e.stopPropagation(); setActiveModal("telemedicine"); }} className="flex items-center gap-1 text-[11px] font-semibold text-primary hover:underline shrink-0">
      ...
      <button
        ...
        className={cn(
          "flex items-center gap-3 w-full rounded-lg border px-4 py-3.5 text-left transition-colors",
          consentPrivacy ? "border-primary bg-primary/10" : "border-border bg-background/50 hover:border-primary/50 hover:bg-background"
        )}
      >
        <Checkbox checked={consentPrivacy} onCheckedChange={() => onAnswer("consentPrivacy", !consentPrivacy)} className="pointer-events-none shrink-0" aria-hidden />
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-[11px] font-bold uppercase tracking-widest text-foreground">Notice of Privacy Practices</span>
          <a href="#" onClick={(e) => { e.preventDefault(); e.stopPropagation(); setActiveModal("privacy"); }} className="flex items-center gap-1 text-[11px] font-semibold text-primary hover:underline shrink-0">
            <ExternalLink className="h-3 w-3" />Click to view
          </a>
          <span className="text-primary font-bold">*</span>
        </div>
      ...
      {!bothChecked && <p className="text-xs text-foreground italic">Both consents are required to submit.</p>}
        ...
        <DrawerPortal>
          ...
          <DrawerContent className="fixed bottom-0 left-0 right-0 z-50 flex flex-col rounded-t-2xl bg-background max-h-[85dvh]">
            <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
              ...
              <button type="button" onClick={() => setActiveModal(null)} className="rounded-full p-1.5 text-foreground/50 hover:text-foreground hover:bg-border/40 transition-colors">
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="px-6 py-4 border-t border-border shrink-0">
              <button type="button" onClick={() => setActiveModal(null)} className="h-11 w-full rounded-lg bg-primary text-primary-foreground font-semibold text-sm hover:bg-primary/90 transition-colors">Close</button>
            </div>
          </DrawerContent>
        </DrawerPortal>
      </Drawer>
